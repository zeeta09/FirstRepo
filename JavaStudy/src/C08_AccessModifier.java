...
public class C08_AccessModifier {
	...
	/*
		...
		- 다른 클래스에서 해당 클래스의 자원을 사용하려고 할 때 어디까지 허용할지를 설정한다.
		...
		- default(아무것도 안 씀)
			: 같은 패키지 내부에서는 자유롭게 접근할 수 있게 된다. 다른 패키지에서는 접근할 수 없다.
			
		- protected
			: 앞에 protected가 붙은 자원은 같은 패키지 내부에서는 자유롭게 접근할 수 있고, 다른 패키지에서는 상속받은 클래스만 접근할 수 있는 자원이 된다.
...
class Choco extends DiffPackage {
	...
	public Choco() {
		// protected : 상속을 받으면 다른 패키지에서도 사용 가능
