...
public class C01_Function {
	...
	/*
		...
		- 정의해둔 함수 이름 뒤에 ()를 붙여서 함수를 호출하면 호출한 자리에 함수의 실행 결과가 반환된다.
	...
	*/
	...
	int : 이 함수의 실행 결과의 타입 (리턴 타입, return type)
	apple_basket : 타입 뒤에 함수명을 정의할 수 있다.
	(int apple, int size) : 이 함수를 실행할 때 int타입 값을 두 개 받도록 정의 (매개 변수, arguments)
	*/
