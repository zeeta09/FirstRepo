...
public class C03_Constructor {
	...
	public static void main(String[] args) {
		...
		System.out.println(orange_box[3]);
		System.out.println(orange_box[4]);
		...
		Book b2 = new Book(700, 30000);
		System.out.printf("%d, %d", b2.pages, b2.price);
	}
}
...
class Orange {
	...
	// 생성자 - 이 클래스의 인스턴스를 생성할 때 가장 먼저 호출되는 메서드. 생성자는 가장 먼저 호출되기 때문에 주로 초기화 용도로 사용된다.
	Orange() {
...
class Strawberry {
	...
	// 다른 오버로딩 타입의 생성자가 존재할때는 기본 생성자를 자동으로 생성해주지 않는다.
	public Strawberry(int seed) {
		this.seed = seed;
	}
}

// myobj에 만들었던 클래스에 알맞은 생성자를 2개 이상 추가해보세요.