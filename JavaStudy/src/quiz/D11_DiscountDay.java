...
public class D11_DiscountDay {
	...
	/*
		어떤 음식점에서 짝수 번째 목요일마다 할인행사를 진행한다고 가정할 때
...
//		System.out.println("입력된 연도는 " + yearEnterd + "입니다!");
	...
	//			event.set(Calendar.MONTH, 1);		
			...
			event.set(Calendar.DAY_OF_WEEK_IN_MONTH, 2);		// 이번 달 네 번째 요일로 설정
			
			System.out.printf(eventFormat.format(event.getTime()) + "\t[%d 번째 행사]\n", count++);
			
			event.set(Calendar.DAY_OF_WEEK_IN_MONTH, 4);		// 이번 달 네 번째 요일로 설정
			
			System.out.printf(eventFormat.format(event.getTime()) + "\t[%d 번째 행사]\n", count++);
			
			// add() : 원하는 필드를 원하는 수치만큼 더하거나 뺄 수있다.
