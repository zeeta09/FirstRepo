...
class Person {
	...
	void sayHi() {
		...
		if (age < 0) {
			...
			System.out.printf("안녕하세요. 내 이름은 %s 입니다. 나이는 %d살 입니다.\n", name, age);
		} else {
			System.out.printf("안녕! 내 이름은 %s이야. 나이는 %d살 이야.\n", name, age);
		}
	}
...
/*
	1. 상속받는 클래스는 생성자에게 가장 먼저 부모의 생성자를 호출해야 한다. 
	(부모의 클래스에 기본생성자가 존재하면 생략할 수 있지만, 존재하지 않을 때는 생략이 불가능하다.)
	
	2. 자식클래스는 부모 클래스의 메서드를 마음대로 고쳐서 사용할 수 있다. (오버라이드, Override)
	
	3. 부모 클래스와 자식 클래스에 똑같은 이름을 가진 자원이 있는 경우에 super와 this로 정확하게 구분할 수 있다.
*/
...
class Police extends Person {
	...
	void arrest(String suspect) {
		System.out.printf("%s 형사는 용의자 %s 씨를 체포했습니다.\n", name, suspect);
	}
}
